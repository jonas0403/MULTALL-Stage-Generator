...
#x_values, r_values, m_prime_values, x0 = channel(h_H)
    ...
    with open(file, "w+") as file:
        ...
        if row == 1:
            ...
            rel_thick = np.array([[0.023, 0.020, 0.015, 0.011, 0.007],
                                      ...
                                      [0.010, 0.009, 0.007, 0.005, 0.003]])# relative values for NACA X4YY
            ...
            file.write("\n")
            file.write("1st to 4th control points for d/l for all levels:\n")
            # d/l max 0,09/0,078/0,06/0,042/0,03
...
def blade_metal_BP(ROW):
    ...
    def find_and_read_data(header_text):
        ...
        #print(f"Debugging1: {header_text}")
        
        for i, line in enumerate(lines):
            if line.strip() == header_text:
                header_found_at = i
                
                #print(f"Debugging2:{header_text}")
                
                break
            
        #if header_found_at == -1:
            #print(f"Debug3:{header_text}")
...
def calculation_of_section_0_5(row):
    ...
    Rtheta_prime_cntr = coa2[124]/coa3[124]
...
def read_parameters_from_file(filename):
    ...
    try:
        with open(filename, 'r') as file:
            for line in file:
                linie = line.strip() # Entfernt die Leerzeichen
                
                if ' = ' in linie:
                    key, value = linie.split(' = ', 1)
                    settings[key] = value
    except FileNotFoundError:
        
        print("Settings Datei '{filename}' nicht vorhanden, Lade Standard Profil...")
...
def run_main_logic(settings):
    ...
    try: 
        levels = [float(x.strip()) for x in levels_input.split(',')] # Liest die Levels ein und wandelt sie in eine Liste von Float-Werten um
    except:
...
class CompressorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Settings for MULTALL Profile")
        ...
        self.save_button = ttk.Button(self.root, text="Save and Initialize", command=self.run_action_and_stay_open) # Button zum Speichern und Initialisieren
    ...
    def open_specification_window(self):
        ...
        section_combo = ttk.Combobox(frame, textvariable=self.specs["section_idx"], values=h_H, state="readonly")
...
#create_multall_dat(NROW, section, levels):

    
def stage_starte_gui():
    root = tk.Tk()
    app = CompressorGUI(root)
    root.mainloop()
