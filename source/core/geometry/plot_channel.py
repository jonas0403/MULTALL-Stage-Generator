# ------------------------------------------------------------------
# File:    source/core/geometry/plot_channel.py
# Author:  Jonas Scholz
# Purpose: Channel-contour plotting.
# ------------------------------------------------------------------
...
def plot_channel(D_S1, D_S2, D_S3, D_H1, D_H2, D_H3, D_M1, D_M2, D_M3,i_st, l_R, l_S, beta_blade_1, beta_blade_2, alpha_blade_2, alpha_blade_3):
   ...
   # Keep track of the total axial offset for multi-stage plotting
    ...
    # crushing smooth intra-stage variation into full-bin cliffs. round(...,4) = 0.1mm.
        ...
        r0_N[1] = round(D_H1[stage_idx]/2, 4) 
        r0_N[2] = r0_N[1] + round((D_H2[stage_idx]-D_H1[stage_idx])/2*(x0[2]-x0[1])/(x0[4]-x0[1]), 4)
        r0_N[0] = round((r0_N[2]-r0_N[1])/(x0[2]-x0[1])*(x0[0]-x0[1])+r0_N[1], 4) # Adjusted for x0 offset
        r0_N[3] = r0_N[1]+ round((D_H2[stage_idx]-D_H1[stage_idx])/2*(x0[3]-x0[1])/(x0[4]-x0[1]), 4)
        r0_N[5] = round(D_H2[stage_idx]/2, 4) + round((D_H3[stage_idx]-D_H2[stage_idx])/2*(x0[5]-x0[4])/(x0[7]-x0[4]), 4)
        r0_N[4] = round((D_H2[stage_idx]/2 + (r0_N[3]+(r0_N[5]-r0_N[3])*(x0[4]-x0[3])/(x0[5]-x0[3])))/2 , 4)
        r0_N[6] = round(D_H2[stage_idx]/2, 4) + round((D_H3[stage_idx]-D_H2[stage_idx])/2*(x0[6]-x0[4])/(x0[7]-x0[4]), 4)
        r0_N[7] = round(D_H3[stage_idx]/2, 4)
        r0_N[8] = round((r0_N[7] - r0_N[6])/(x0[7] - x0[6])*(x0[8] - x0[7]) + r0_N[7], 4)
        
        
        
        r0_G = np.full_like(t0, 0.0)
        r0_G[1] = round(D_S1[stage_idx]/2, 4)
        r0_G[2] = r0_G[1] + round((D_S2[stage_idx]-D_S1[stage_idx])/2*(x0[2]-x0[1])/(x0[4]-x0[1]), 4)
        r0_G[0] = round((r0_G[2]-r0_G[1])/(x0[2]-x0[1])*(x0[0]-x0[1])+r0_G[1], 4) # Adjusted for x0 offset
        r0_G[3] = r0_G[1]+ round((D_S2[stage_idx]-D_S1[stage_idx])/2*(x0[3]-x0[1])/(x0[4]-x0[1]), 4)
        r0_G[5] = round(D_S2[stage_idx]/2, 4) + round((D_S3[stage_idx]-D_S2[stage_idx])/2*(x0[5]-x0[4])/(x0[7]-x0[4]), 4)
        r0_G[4] = round((D_S2[stage_idx]/2 + (r0_G[3]+(r0_G[5]-r0_G[3])*(x0[4]-x0[3])/(x0[5]-x0[3])))/2 , 4)
        r0_G[6] = round(D_S2[stage_idx]/2, 4) + round((D_S3[stage_idx]-D_S2[stage_idx])/2*(x0[6]-x0[4])/(x0[7]-x0[4]), 4)
        r0_G[7] = round(D_S3[stage_idx]/2, 4)
    ...
    # [FIX kink] single global hub/shroud spline across stages (mirrors the grid global
    # annulus resampling in init_channel_data): shared endpoints give positions, one spline
    # gives matching slopes. Per-stage arrays below are resampled from it; mirrors, meanline
    # and channel-height lists derive afterwards and inherit the smooth curves.
    _gx_h, _gr_h, _gx_s, _gr_s = [], [], [], []
    for _s in range(i_st):
        _x0 = all_x0_stages[_s]
        _rN = all_r0_N_stages[_s]
        _rG = all_r0_G_stages[_s]
        _start = 0 if _s == 0 else 2
        _end = 8 if _s == i_st - 1 else 7
        for _i in range(_start, _end + 1):
            ...
            _gr_s.append(_rG[_i])
    for _s in range(i_st):
        _xs = all_x_N_stages[_s]
        _rN = [cubspline(3, _x, _gx_h, _gr_h) for _x in _xs]
        _rG = [cubspline(3, _x, _gx_s, _gr_s) for _x in _xs]
        all_r_N_stages[_s] = _rN
        all_r_G_stages[_s] = _rG
        all_meanline_stages[_s] = [(_a + _b) / 2 for _a, _b in zip(_rG, _rN)]
        all_channelHeight_stages[_s] = [_a - _b for _a, _b in zip(_rG, _rN)]

    # Mirroring of Shroud and Hub radii (R_G, R_N) - for plotting below axis
    # These will now be lists of lists
    all_R_G_stages = []
    all_R_N_stages = []
    # [FIX bug #1] radii are computed in meters but axial coords + axis label are mm;
    # convert all plotted radii-like arrays to mm (x arrays stay untouched).
    # Mirrored (R_N/R_G) and combined-scatter lists derive below and inherit mm.
    all_r0_N_stages = [[v * 1000.0 for v in st] for st in all_r0_N_stages]
    all_r0_G_stages = [[v * 1000.0 for v in st] for st in all_r0_G_stages]
    all_r_N_stages = [[v * 1000.0 for v in st] for st in all_r_N_stages]
    all_r_G_stages = [[v * 1000.0 for v in st] for st in all_r_G_stages]
    all_meanline_stages = [[v * 1000.0 for v in st] for st in all_meanline_stages]
    all_channelHeight_stages = [[v * 1000.0 for v in st] for st in all_channelHeight_stages]
